...
if __name__ == "__main__":
    ...
    dt = time.time()-t0
    print(f"dt={dt*1e9:.1f}nsecs - " +
          f"find_nearest() - range at {angles[ind]:.2f}:" +
          f" {ranges[ind]:.2f}")

    test_cases = [{'angle': 181.0,
                   'wrap_result': -179.0,
                   },
                  {'angle': 0.0,
                   'wrap_result': 0.0,
                  ...
                  {'angle': np.asarray(range(0, 360, 30)),
                   'wrap_result': np.asarray([0.0, 30.0, 60.0, 90.0, 120.0, 150.0, -180.0, -150.0, -120.0, -90.0, -60.0, -30.0]),
                   },
                  ]
    ...
    for item in test_cases:
        ...
        assert (np.round(wrapped_angle_i, 5) == item['wrap_result']).all(), should_be_msg

...
class cTurtle():
    ...
    def scanCB(self, message):
        self.front_vision = message.ranges[-half_front_vision_range:] + \
            message.ranges[:half_front_vision_range]

    def move_straight(self):
        print('moving straight\t[%.2f,\t%.2f,\t%.2f]' % (self.front_vision[0],
              self.front_vision[self.center_ray_index], self.front_vision[-1]))
        if np.isinf(self.front_vision[self.center_ray_index]):
            self.vel_msg.linear.x = math.sqrt(3.5-min_dist)*kpLinear+0.05
        else:
            self.vel_msg.linear.x = math.sqrt(np.min(
                [self.front_vision[-1], self.front_vision[self.center_ray_index], self.front_vision[0]])-min_dist)*kpLinear+0.06
    ...
    def turn(self):
        ...
        if self.front_vision[-1] < self.front_vision[0]:
            while self.front_vision[-1] < delimiting_sensor_range:
                ...
                print('turning right\t[%.2f,\t%.2f,\t%.2f]' % (
                    self.front_vision[0], self.front_vision[self.center_ray_index], self.front_vision[-1]))
            ...
            while self.front_vision[0] < delimiting_sensor_range:
                self.rot_right_msg.angular.z = (
                    (delimiting_sensor_range-self.front_vision[0])*kpAngular+0.05)
                print('turning left\t[%.2f,\t%.2f,\t%.2f]' % (self.front_vision[0],
                      self.front_vision[self.center_ray_index], self.front_vision[-1]))
    ...
    def core(self):
        ...
        if len(self.front_vision):
            ...
            if self.front_vision[self.center_ray_index] <= min_dist or self.front_vision[-1] <= min_dist or self.front_vision[0] <= min_dist:
                while self.Vx > 0.02:
                    self.stop_moving()
                    self.rate.sleep()
                self.turn()

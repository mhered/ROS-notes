...
Make linear velocity proportional to the distance to obstacles - similar to
 Go-to-Goal Behavior

Make angular velocity proportional to the distance to obstacles on the left
 and right sides:
- rotate smoothly to the right if obstacles on the left are much closer than
on the right.
- rotate smoothly to the left in the opposite case

Test on Turtlebot3 in the maze and house environments, and on a real robot.
...
def smooth_bumper_app(velocity_publisher):
    """ Smooth Bumper robot based on Go to goal method """
...
if __name__ == '__main__':
    try:
        ...
        # call the smooth bumper app
        smooth_bumper_app(velocity_publisher)

...
if __name__ == "__main__":
    ...
    turtle_follower_velocity = rospy.Publisher(
    ...
    while not rospy.is_shutdown():
        try:
            ...
            (translation, rotation) = transform_listener.lookupTransform(
                "/turtle2_frame", "/turtle1_frame", rospy.Time(0)
            )
        except (
            tf.LookupException,
        ...
        angular = 4 * math.atan2(
            y_follower_in_turtle1_frame, x_follower_in_turtle1_frame
        )
        linear = 0.5 * math.sqrt(
            x_follower_in_turtle1_frame**2 + y_follower_in_turtle1_frame**2
        )
